 ...
 * </p>
 */
public class JBPMSimplePager extends AbstractPager {
    ...
    public JBPMSimplePager(TasksListPresenter presenter) {
        this();
    ...
    public JBPMSimplePager(TextLocation location) {
     ...
     * @param showLastPageButton if true, show a button to go the the last page
     */
    public JBPMSimplePager(TextLocation location, boolean showFastForwardButton, final int fastForwardRows, boolean showLastPageButton) {
     ...
     */
    public JBPMSimplePager(TextLocation location, Resources resources, boolean showFastForwardButton, final int fastForwardRows, boolean showLastPageButton, ImageButtonsConstants imageButtonConstants) {
     ...
     * @param showLastPageButton if true, show a button to go the the last page
     */
    public JBPMSimplePager(TextLocation location, Resources resources, boolean showFastForwardButton, final int fastForwardRows, boolean showLastPageButton) {
    ...
    public boolean hasNextPage() {
        if (presenter != null && presenter.getAllTaskSummaries() != null) {
           
            return (presenter.getDataProvider().getList().size() > (DataGridUtils.pageSize * (currentPage + 1)) );
        }
        return false;
    }
    ...
    public void nextPage() {
        ...
        int pageSizeLocal = range.getLength();
        int dataSize = display.getRowCount();
        int endIndex = Math.min(dataSize, pageStart + pageSizeLocal - 1);
       
        
        if (presenter != null && (endIndex > (dataSize - this.pageSize - 2))) {
            presenter.refreshTasks(presenter.getCurrentDate(), presenter.getCurrentView(), 
                    presenter.getCurrentTaskType(), ((currentPage + 1) * this.pageSize), 
                    this.pageSize , false);
        }
        super.nextPage();
 ...
 //@TODO:  I need to create a method to remove data from the server side and client side     
//        if (presenter != null) {
//            presenter.refreshTasks(presenter.getCurrentDate(), presenter.getCurrentView(), presenter.getCurrentTaskType(), (currentPage * DataGridUtils.pageSize), DataGridUtils.pageSize * 3);
//        }
        super.previousPage();
    }
